...
namespace AutoFillForm
{
    class Daype : IWebSites
    {
        ...
        public void carpostfunc(System.Windows.Forms.WebBrowser webBrowser1, IList<AutoFillForm.com.unitedcarexchange.UsedCarsInfo> obUsedCarsInfo, bool a)
        {
            ...
            {
                title = obUsedCarsInfo[0].YearOfMake.ToString() + " " + obUsedCarsInfo[0].Make.ToString() + " " + obUsedCarsInfo[0].Model.ToString();
            }
            else
                title = obUsedCarsInfo[0].YearOfMake.ToString() + " " + obUsedCarsInfo[0].Make.ToString();
        ...
        private string WrapTextByMaxCharacters(string details, string dep, int val, string pn)
        {
