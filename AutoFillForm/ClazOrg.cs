...
{
    public class ClazOrg : IWebSites
    {
     ...
     public void carpostfunc(WebBrowser webBrowser1, IList<com.unitedcarexchange.UsedCarsInfo> obUsedCarsInfo,bool a)
     {
         ...
         string pricenew = "$"+obUsedCarsInfo[0].Price.ToString();

         GeneralFunction.SetTextValue(webBrowser1, "price", pricenew);
               // GeneralFunction.SetDivValuebyClass(webBrowser1, "nicEdit-main", obUsedCarsInfo[0].Description.ToString());
                ...
                string state = obUsedCarsInfo[0].State.ToString();
                string statename = StateName.GetStateName(obUsedCarsInfo, obUsedCarsInfo[0].State.ToString(), obUsedCarsInfo[0].Carid.ToString());
                string statefrmt = statename + "," + state;
                GeneralFunction.SetTextValue(webBrowser1, "city", obUsedCarsInfo[0].Zipcode.ToString());
                GeneralFunction.LinkInvoke(webBrowser1, "Used Cars");
                GeneralFunction.SetTextValue(webBrowser1, "category", "Used Cars");
     ...
     string WrapTextByMaxCharacters(string details,string objText, int intMaxChars, string url, string phone)
        {
            ...
            if (objText != null)
            {
                if (objText.ToString().Trim() != "")
                {
                    ...
                    {
                        strReturnValue = details.ToString() + "\r\n" + "\n" + "Description: " + objText.ToString().Trim() + "..!! If instrested contact : " + phone + "..!!!For More Details:  " + url;
                    }
                    else
                    {
                        strReturnValue = details.ToString() + "\r\n" + "\n" + "Description: " + objText.ToString().Trim() + "..!! If instrested contact : " + phone + "..!!!For More Details:  " + url;
