...
{
   public class classifieds4me :IWebSites
    {
       ...
       public void carpostfunc(WebBrowser webBrowser1, IList<com.unitedcarexchange.UsedCarsInfo> obUsedCarsInfo,bool a)
       {
           ...
           if (obUsedCarsInfo[0].Title != "Emp")
           {
               ...
               {
                   int b = title.IndexOf(' ');
                   title = title.Substring(b).Trim();
               }
           }
           else
           {
               if (obUsedCarsInfo[0].Model.ToString() != "Other")
                   title = obUsedCarsInfo[0].Make.ToString() + "  " + obUsedCarsInfo[0].Model.ToString();
               //+ " - " + "$"+obUsedCarsInfo[0].Price.ToString();obUsedCarsInfo[0].YearOfMake.ToString() + "  " + ;
               else
                   title = obUsedCarsInfo[0].Make.ToString();
               //+ " - " + "$"+obUsedCarsInfo[0].Price.ToString();obUsedCarsInfo[0].YearOfMake.ToString() + "  " + 
           }
          // title = obUsedCarsInfo[0].Make.ToString() + " " + obUsedCarsInfo[0].Model.ToString();
           GeneralFunction.SetTextValue(webBrowser1, "ctl00$ContentPlaceHolder1$txtSubject", title);

           GeneralFunction.SetTextValue(webBrowser1, "ctl00$ContentPlaceHolder1$txtOptionalText1", obUsedCarsInfo[0].YearOfMake.ToString());
           if (obUsedCarsInfo[0].Price.ToString()=="0")
           {
                GeneralFunction.SetTextValue(webBrowser1, "ctl00$ContentPlaceHolder1$txtOptionalText2", "1");
           }
           else
           {


           GeneralFunction.SetTextValue(webBrowser1, "ctl00$ContentPlaceHolder1$txtOptionalText2", obUsedCarsInfo[0].Price.ToString());
           }
           string location = obUsedCarsInfo[0].City.ToString() + "," + obUsedCarsInfo[0].State.ToString() + "," + obUsedCarsInfo[0].Zip.ToString();
           GeneralFunction.SetTextValue(webBrowser1, "ctl00$ContentPlaceHolder1$txtLocation", location);
          ...
          // string fule = obUsedCarsInfo[0].Fueltype.ToString() == "Emp" ? "" : obUsedCarsInfo[0].Fueltype.ToString() == "Unspecified" ? "" : "\r\nFuelType: " + obUsedCarsInfo[0].Fueltype.ToString();
           ...
           dep = dep.Replace("\n", " ");
           string URLDesp = details.ToString() + "\r\n" + "\n" + "Description: " + dep.Trim() + "..!! If intrested contact : " + pn + "..!!!For More Details:  " + url;
               //WrapTextByMaxCharacters(details, dep, val, url, pn);

           GeneralFunction.SetMultiTextValue(webBrowser1, "ctl00$ContentPlaceHolder1$txtMessage", URLDesp);

           GeneralFunction.SetTextValue(webBrowser1, "ctl00$ContentPlaceHolder1$txtEmail", Globaltxtemail.txtemail);
