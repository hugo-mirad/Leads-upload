...
{
    public class classifiedsvalley : IWebSites
    {
   ...
   public  string statename;
           ...
           
        {
            ...
            //string stname = statename1;
            statename1 = statename1.ToLower();
            string str = statename1;
            str = str.Replace(" ", "");
            if (str == "oklahoma")
                str = "oregon";
            Globalstname.stname = str.ToLower().Trim();
            if (webBrowser1.Url.ToString() == "http://www.classifiedsvalley.com/")
            {
                //GeneralFunction.LinkInvoke(webBrowser1, "+ Change State ");
                
                statename = StateName.GetStateName(obUsedCarsInfo, obUsedCarsInfo[0].State.ToString(), obUsedCarsInfo[0].Carid.ToString());
                

                GeneralFunction.LinkInvoke(webBrowser1, statename);
            }

            string state = "http://www.classifiedsvalley.com/" + Globalstname.stname + "/";

            if (webBrowser1.Url.ToString() == state)
            {
            ...
            GeneralFunction.SetTextValue(webBrowser1, "area", location);
