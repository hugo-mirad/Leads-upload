...
{
   public class Adsriver :IWebSites
   {
       ...
       public void carpostfunc(WebBrowser webBrowser1, IList<com.unitedcarexchange.UsedCarsInfo> obUsedCarsInfo,bool a)
       {
           ...
           string state = obUsedCarsInfo[0].State.ToString() + "-" + obUsedCarsInfo[0].City.ToString();
           GeneralFunction.SetTextValue(webBrowser1, "area", state);
           ...
           string URLDesp = details.ToString() + "\r\n" + "\n" + "Description: " + dep.Trim() + "..!! If instrested contact : " + pn + "..!!!For More Details:  " ;
               //WrapTextByMaxCharacters(details, dep, val, url, pn);
           if (details.Contains("Emp"))
           {
               details = details.Replace("Emp", "");
           }
           GeneralFunction.SetMultiTextName(webBrowser1, "addesc", URLDesp + url);
